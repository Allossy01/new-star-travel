    ...
    <section id="home" style={{ position: 'relative', minHeight: '100vh', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', overflow: 'hidden' }}>
      ...
      <div style={{
        ...
        <motion.h1 initial={{ opacity: 0, y: 40 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.9, delay: 0.4 }}
          style={{ fontSize: 'clamp(32px, 5.5vw, 66px)', fontWeight: 900, color: '#fff', lineHeight: 1.15, margin: '0 0 12px', width: '100%' }}>
          {t.hero.headline.split(' ').slice(0, 6).join(' ')}{' '}
          <span style={{ background: 'linear-gradient(135deg, #b8960c, #d4af37, #b8960c)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent', backgroundClip: 'text' }}>
        ...
        <motion.p initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8, delay: 0.6 }}
          style={{ color: 'rgba(255,255,255,0.68)', fontSize: 'clamp(15px, 1.8vw, 19px)', maxWidth: 620, margin: '0 auto 40px', lineHeight: 1.75 }}>
          {t.hero.subheadline}
        </motion.p>
        ...
        <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8, delay: 0.8 }}
          ...
          <motion.button
            ...
            style={{
              ...
              borderRadius: 99, fontWeight: 700, fontSize: 16, border: 'none',
              boxShadow: '0 6px 28px rgba(208,0,0,0.45)', cursor: 'pointer',
              fontFamily: 'inherit',
            }}
      ...
      <motion.div style={{ position: 'absolute', bottom: 32, left: '50%', transform: 'translateX(-50%)' }}
        ...
        <div style={{ width: 24, height: 40, border: '2px solid rgba(255,255,255,0.25)', borderRadius: 12, display: 'flex', justifyContent: 'center', paddingTop: 6 }}>
          <motion.div style={{ width: 4, height: 8, background: '#b8960c', borderRadius: 2 }}
